...
function displaySongs(song) {
  ...
  if (song.audioFiles != "") {
    ...
    audio_button.setAttribute(
      "onclick",
      `function toggleAudio(){const thisList = document.querySelector('#${song.file}List'); thisList.style.display == 'none' ? thisList.style.display = 'block' : thisList.style.display = 'none'};toggleAudio()`
    );
